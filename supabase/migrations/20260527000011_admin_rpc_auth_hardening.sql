...
as $$
  ...
  from "OrderFeedback"
  where app_admin_can_access(p_restaurant_id)
...
as $$
  with authz as (
    select
      ...
      or p_group_owner_id = auth.uid()::text
      or exists (
        select 1
        from "User" u
        join "Restaurant" r on r.id = u.restaurant_id
        where u.id = auth.uid()::text
          and u.role = 'owner'
          and r.group_owner_id = p_group_owner_id
      ) as allowed
  ),
  restaurants as (
    select id, name
    ...
    where (select allowed from authz)
      and group_owner_id = p_group_owner_id
  ),
  today_orders as (
